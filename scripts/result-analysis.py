...
def search(result_path, network_type, num_layers,
  ...
  if num_layers is not "all":
    all_layers=[int(num_layers)];
  if num_neurons is not "all":
    all_neurons=[int(num_neurons)];
  if training_method is not "all":
    all_methods=[training_method];
  if regularization is not "all":
    ...
    for n_neurons in all_neurons:
      for method in all_methods:
        for regular in all_regularization:
          ...
          with open(result_path+exp_id+".pkl", 'r') as f:
            result_list.append(pickle.load(f));
  
  return result_list;
  
def analysis(result_path, network_type, num_layers,
             num_neurons, batch_size, num_epochs,
  ...
  if mode is "targets-predicted":
    assert num_layers is not "all", "num-layers should be 1-5 in targets-predicted mode";
    assert num_neurons is not "all", "num-neurons should be 10-300 in targets-predicted mode";
    assert training_method is not "all", "training-method shouldn't be all in targets-predicted mode";
    assert regularization is not "all", "regularization shouldn't be all in targets-predicted mode";
    ...
    draw.draw_target_predicted(result['targets'], result['predicted'], result['exp_id']+"targets-predicted");
  elif mode is "epochs-cost":
    assert num_layers is not "all", "num-layers should be 1-5 in epochs-cost mode";
    assert num_neurons is not "all", "num-neurons should be 10-300 in epochs-cost mode";
    assert training_method is not "all", "training-method shouldn't be all in epochs-cost mode";
    assert regularization is not "all", "regularization shouldn't be all in epochs-cost mode";
    results_list=search(result_path, network_type, num_layers, num_neurons,
                        batch_size, num_epochs, training_method, regularization);
    result=results_list[0];
    draw.draw_epochs_cost(result['cost'], result['exp_id']+"epochs-cost");
  elif mode is "cost-algorithm": 
    assert num_layers is not "all", "num-layers should be 1-5 in cost-algorithm mode";
    assert num_neurons is not "all", "num-neurons should be 10-300 in cost-algorithm mode";
    assert training_method is "all", "training-method should be all in cost-algorithm mode";
    assert regularization is not "all", "regularization shouldn't be all in cost-algorithm mode";
    results_list=search(result_path, network_type, num_layers, num_neurons,
                        batch_size, num_epochs, training_method, regularization);
    ...
    draw.draw_cost_algorithms(cost_arr, 
                              ds.create_exp_id(exp_network, num_layers, num_neurons, 
                                               batch_size, num_epochs, "all", regularization)+"cost-algorithm");
  elif mode is "neurons-cost":
    assert num_layers is "all", "num-layers should be all in neurons-cost mode";
    assert num_neurons is "all", "num-neurons should be all in neurons-cost mode";
    assert training_method is not "all", "training-method shouldn't be all in neurons-cost mode";
    ...
    draw.draw_neurons_layers_cost(cost, 
                                  ds.create_exp_id(exp_network, "all", "all", batch_size, 
                                                   num_epochs, learning_method, regularization)+"neurons-cost");
  elif mode is "cost-regular":
    assert num_layers is not "all", "num-layers should be 1-5 in cost-regular mode";
    assert num_neurons is not "all", "num-neurons should be 10-300 in cost-regular mode";
    assert training_method is not "all", "training-method shouldn't be all in cost-regular mode";
    assert regularization is "all", "regularization should be all in cost-regular mode";
    results_list=search(result_path, network_type, num_layers, num_neurons,
                        batch_size, num_epochs, training_method, regularization);
    cost_arr=results_list[0]['cost'][1,:];
    ...
    draw.draw_cost_dropout(cost_arr,
                           ds.create_exp_id(exp_network, num_layers, num_neurons, 
                                            batch_size, num_epochs, learning_method, "all")+"cost-regular");
  return ;
...
parser.add_argument("--num-layers", type=str, default="1",
                    help="Number of hidden feedforward layers.");
